...
3：UDP比TCP更省带宽？ 错。 一个没有实现 稳定/可靠/兼容性极好的 UDP，确实包头长度小于TCP, 但如果UDP要实现 稳定/可靠/兼容性好 的话，这个优势就不存在了。
...
(QUIC/UDP 夸张说法, 解决了队头阻塞的问题。其实就是多个流，多个流互不干扰，自然不存在队头阻塞这个问题。但流内部依然会存在队头阻塞。一般开发中，1个流足够了)
